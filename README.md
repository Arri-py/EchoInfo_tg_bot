...
- В личке: если отправить кастомную emoji из Premium-набора, бот вернет ее `custom_emoji_id`.
...
- Aiogram 3.4.1 не умеет получать список топиков и не содержит `get_forum_topic`; бот выводит сведения о топике только если команда вызывается внутри него и в сообщении есть `forum_topic_created` (иначе покажет только thread_id).

## Troubleshooting
- Команда в канале не отвечает: убедитесь, что бот добавлен как админ (или с правом читать посты) и команда написана через `@username`.
- В группах не видно админов/счётчика участников: дайте боту право «Просматривать список участников».
- В форумах бот показывает только thread_id: Telegram не вернул `forum_topic_created` — вызовите `/info` прямо в нужной ветке, либо создайте новый пост в топике и повторите.
- Пришло уведомление об ошибке владельцу: смотрите тип исключения в сообщении; бот уже перезапустил polling автоматически.
